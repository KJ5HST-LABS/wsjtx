...
- Broken into four sub-phases, each one session per `PHASE_3_TESTING_PLAN.md`:
  ...
  - **3d — Vendor + catalog close.** Steve's `decoder_tests.bash` + `decoder_test_results_v3.0.1.txt` vendored under `tests/decoders/franke/reference/`. `tests/decoders/franke/README.md` describes the corpus and the script-to-catalog translation. Attribution draft removed (Steve confirmed not needed). Commit `ece547850`; CI run `24579224586` green on all four platforms.
